(function () {
  ...
  shadow.innerHTML = '<style>' +
    ':host{all:initial;position:fixed;inset:0 0 auto 0;height:80px;z-index:2147483647;display:block;font:13px -apple-system,BlinkMacSystemFont,\"Segoe UI\",sans-serif}' +
    '.chrome{height:80px;box-sizing:border-box;background:#181920;color:#f8f8f2;border-bottom:1px solid #ffffff24;box-shadow:0 2px 10px #0006}' +
    ...
    '.bar{height:46px;box-sizing:border-box;display:flex;align-items:center;gap:6px;padding:6px 9px;background:#181920}' +
    ...
    '<div class=\"bar\" role=\"toolbar\" aria-label=\"Target browser controls\">' +
  ...
  function update() {
    var active = (tabState.tabs || []).find(function (tab) { return tab.id === tabState.active_id; });
    urlInput.value = active ? active.url : location.href;
  ...
  shadow.getElementById('go').addEventListener('submit', function (event) {
    event.preventDefault();
    var next = validUrl(urlInput.value.trim());
    if (!next) {
      showError('Gunakan URL http:// atau https://');
      return;
    }
    location.href = next;
  });
