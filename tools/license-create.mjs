...
if (!args.email || !args['private-key']) throw new Error('Usage: npm run license:create -- --email customer@example.com --days 2 --private-key ./license-keys/private.pem');
const email = args.email.trim().toLowerCase(); const days = Number(args.days || 2); if (!Number.isFinite(days) || days <= 0) throw new Error('--days harus positif');
